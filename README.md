...
- 取當日**漲幅 > 8%**(`STRONG_THRESHOLD`,漲停 10%,>8% 視為準漲停強勢股)的個股,
  做產業價值鏈族群分類,統計各族群的強勢股家數。
- 依**強勢股家數**排序,並顯示**強勢比例**(家數 ÷ 族群當日家數)供判斷——
  比例高代表整個族群在噴,而非大族群隨機湊到幾檔。
- 入榜門檻:強勢股家數 ≥ 2,且族群成員數 ≥ `MIN_GROUP_SIZE`(排除單股/迷你族群)。
- **只看當天、不需歷史累積**,當日盤後即有意義;點開可看該族群的強勢個股清單。
