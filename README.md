...
- 預設依**強勢股家數**排序,並顯示**強勢比例**(家數 ÷ 族群當日家數)——
  比例高代表整個族群在噴,而非大族群隨機湊到幾檔。**點欄位標題可改排序**
  (家數/比例/族群家數/層級),點列展開該族群的強勢個股清單。
- 入榜門檻:強勢股家數 ≥ 2,且族群成員數 ≥ `MIN_GROUP_SIZE`(排除單股/迷你族群)。
- **只看當天、不需歷史累積**,當日盤後即有意義。
- **層級標記與篩選**:每個族群標示 鏈 / 主分類 / 細分類,可用按鈕篩選層級。
  鏈是整條產業鏈(最廣),底下有主分類與細分類。例:「被動元件(鏈)」涵蓋
  「電容器、電感器、電阻器(主分類)」——大傘族群家數多但會稀釋,改看主/細分類
  或依強勢比例排序,熱區子族群(如電容器)即浮現。
- **題材聚焦(AI/衛星等)**:以官方全部 44 條產業鏈(鏈層級族群)為題材清單。
  每檔強勢股會標上所屬的全部題材徽章(一檔可屬多條鏈),上方「題材聚焦」選單
  可選一個題材(選項取自當日實際有強勢股的題材),系統將含該題材強勢股的族群
  置頂、其餘淡化,並把該題材的個股高亮——即可看出「今天在噴的東西有哪些跟
  AI / 衛星 / 電動車…相關」,即使它們散落在半導體、連接器、醫療器材等不同族群。
